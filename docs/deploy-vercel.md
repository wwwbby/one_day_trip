...
把 Daytrip Planner 部署到 Vercel。流程参考 `gemini-rss-app` 的 Vercel + 环境变量方式；本项目的一日规划已经支持服务端持久化，部署后建议配置 Neon/Postgres 的 `DATABASE_URL`，这样电脑和手机能看到同一份规划。

## 你需要准备

- GitHub 账号
- Vercel 账号
- Neon/Postgres 数据库连接串：给一日规划跨设备同步用
...
| `DATABASE_URL` | 必填 | Neon/Postgres 连接串，用于保存一日规划。服务端会自动创建 `daytrip_plans` 表。 |
...
4. 测试：
   ...
   - Anitabi 导入是否工作
   - 两个点以上的自动规划是否能返回路线
   - 新建一个规划后，用手机打开同一个 Vercel 域名是否能看到

## 已适配的代码结构

```text
vercel.json                         Vercel 构建、输出目录、函数超时配置
api/plans.js                        /api/plans
api/plans/[id].js                   /api/plans/:id
...
- 当前一日规划会通过 `/api/plans` 保存到服务端。配置 `DATABASE_URL` 时使用 Neon/Postgres；本地开发没有数据库时会写入 `.data/daytrip-plans.json`。Vercel 上必须配置 `DATABASE_URL`，不能依赖函数本地文件。
- 浏览器 `localStorage` 只作为缓存和旧数据迁移来源。第一次连上服务端且服务端为空时，应用会把本地缓存里的旧规划上传一次。
...
现在所有访问同一部署域名的人会看到同一组一日规划，适合个人自用原型。要做多用户产品，可以继续扩展：

1. 接入登录。
2. 给 `daytrip_plans` 增加 `user_id` 或 `workspace_id`。
3. 在 `/api/plans` 查询和保存时按用户隔离。
